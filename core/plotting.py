...
def plot_data(config,
    ...
    if spectrum_contrem_plot:
        ...
        if model_CC_wlen is not None:
            ...
            model_CC_wlen_renamed = {'Retrieved ' + key:model_CC_wlen[key] for key in model_CC_flux.keys()}
            ax = custom_plot(ax,model_CC_wlen_renamed,model_CC_flux_renormed,color='r',lw = 0.5)
...
def custom_errorbar(ax,x,y,xerr,yerr,reduce_resolution = True,**kwargs):
    ...
    if isinstance(x,dict):
        if np.sum([len(x[key]) for key in x.keys()]) > 1000 and reduce_resolution:
            reduce_res = True
        for key in x.keys():
            if xerr is None:
                xerr_temp = None
        ...
        if len(x) > 5000 and reduce_resolution:
            reduce_res = True
        if reduce_res:
            xerr_plot = xerr_temp
            if xerr_temp is not None:
            ...
            if yerr_temp is not None:
                yerr_plot = yerr_temp[10::]
            ax.errorbar(x[10::],y[10::],xerr = xerr_plot,yerr = yerr_plot,**kwargs)
        else:
            ax.errorbar(x,y,xerr = xerr,yerr = yerr,**kwargs)
    return ax


def custom_plot(ax,x,y,reduce_resolution = True,**kwargs):
    reduce_res = False
    if isinstance(x, dict):
        if np.sum([len(x[key]) for key in x.keys()]) > 1000 and reduce_resolution:
            reduce_res = True
        ...
        if len(x) > 1000 and reduce_resolution:
            reduce_res = True
        if reduce_res:
            ax.plot(x[10::],y[10::],**kwargs)
        else:
            ax.plot(x,y,**kwargs)
    return ax
...
def plot_retrieved_spectra_FM_dico(
    ...
    if 'log_R' not in config['PARAMS_NAMES'] and 'R' in config['PARAMS_NAMES']:
        ...
        if 'R' in config['PARAMS_NAMES']:
            index_R = config['PARAMS_NAMES'].index('R')
            samples[:,index_R] = np.log10(samples[:,index_R])
            config['PARAMS_NAMES'][index_R] = 'log_R'
        if 'R' in config['ALL_PARAMS']:
            index_R = config['ALL_PARAMS'].index('R')
            config['ALL_PARAMS'][index_R] = 'log_R'
        if 'R' in config['TEMPS']:
            index_R = config['TEMPS'].index('R')
            config['TEMPS'][index_R] = 'log_R'
        if 'R' in config['DATA_PARAMS'].keys():
            config['DATA_PARAMS']['log_R'] = np.log10(config['DATA_PARAMS']['R'])
            del config['DATA_PARAMS']['R']
...
def plot_retrieved_spectra_ck(
    ...
    if nb_picks > 0:
        ...
        for pos_i in range(nb_picks):
            ax.plot(wlen_median,flux_curves[pos_i]*scale_factor,color=color,lw=0.1,alpha=0.2,zorder=zorder)
    
    ax.plot(wlen_median,flux_median*scale_factor,color=color,lw=lw,label='Best-fit',alpha=alpha,zorder=zorder)
